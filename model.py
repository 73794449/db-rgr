...
def verify_value(selected_table, selected_param, entered_param):
    ...
    if ((selected_table == 1 and selected_param == 3) and
        ...
        return False
    if ((selected_table == 4 and selected_param == 2) or (selected_table == 5
                                                          and selected_param == 4)
            or (selected_table == 6 and selected_param == 4)):
        try:
            check_date = bool(datetime.strptime(entered_param, format_for_date))
        except ValueError:
            check_date = False
        return check_date
    if selected_table == 4 and selected_param == 3:
        try:
...
class Model:
    ...
    tables_for_rand = {
        1: f"INSERT INTO \"BloodBag\" ( \"BloodType\", \"StorageTemperature\") VALUES "
           f"((array['A+', 'O+', 'B+','AB+', 'A-', 'O-', 'B-', 'AB-'])[floor(random()*8+1)], floor(random()*3 + 18))",
        ...
        3: f"INSERT INTO \"BloodBank\" (\"Name\", \"Location\", \"ContactNumber\", \"TotalDonations\","
           ...
           f"(array['HelpAnother', 'BeDonor', 'Back4Blood','Bloody', 'MealsForBlood', 'MeatHook', "
           f"'TheCakeislie', 'Bloodborn'])[floor(random()*8+1)],"
           f"(array['Ukraine', 'USA', 'France','Germany', 'UK', 'Poland', 'Canada', 'India'])[floor(random()*8+1)],"
           f"to_char(random() * 10000000000, 'FM\"(\"000\") \"000\"-\"0000'),"
           f"floor(random()*1000+10),"
           f"(SELECT \"DonationID\" FROM \"BloodDonation\" ORDER BY RANDOM() LIMIT 1))",
        4: f"INSERT INTO \"BloodDonation\" (\"DonationDate\", \"DonationTime\",\"DonorID\",\"DonationStatus\") VALUES("
        ...
        5: f"INSERT INTO \"Donor\" (\"FirstName\", \"LastName\", \"DateOfBirth\", \"ContactNumber\", \"BloodType\") "
           f"VALUES"
           f"((array['Ivan', 'Borys', 'Jorik','Olesia', 'Patrick', 'John', 'Steven', 'Nastya'])[floor(random()*8+1)],"
           f"(array['Terentiev', 'Zorev', 'Smith','Miller', 'Williams', 'Davis', 'Steven', 'Garcia'])[floor(random("
           f")*8+1)],"
           f"date '2077-01-01' + (random() * (date '2000-11-11' - date '2077-01-01'))::int,"
           f"to_char(random() * 10000000000, 'FM\"(\"000\") \"000\"-\"0000'),"
        ...
        6: f"INSERT INTO \"Recipient\" (\"FirstName\", \"LastName\", \"DateOfBirth\","
           ...
           f"((array['Ivan', 'Bora', 'Jorik','Olesia', 'Patrick', 'John', 'Steven', 'Nastya'])[floor(random()*8+1)],"
           f"(array['Terentiev', 'Zorev', 'Smith','Miller', 'Williams', 'Davis', 'Steven', 'Garcia'])[floor(random("
           f")*8+1)],"
           f"date '2077-01-01' + (random() * (date '2000-11-11' - date '2077-01-01'))::int,"
           f"to_char(random() * 10000000000, 'FM\"(\"000\") \"000\"-\"0000'),"
    ...
    def edit_table(self, selected_table, selected_id, selected_param, entered_param):
        ...
        Verification = verify_value(selected_table, selected_param, entered_param)
        if Verification:
